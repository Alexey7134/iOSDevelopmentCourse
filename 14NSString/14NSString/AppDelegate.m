...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    */
    ...
    for (NSString* element in array) {
        if (![[element substringFromIndex:[element length]-1] isEqualToString:@"."]) {
            [arraySentence addObject:[element stringByAppendingString:@"."]];
        } else {
            [arraySentence addObject:element];
        }
    }
    
    NSMutableArray* arraySentenceWithString = [[NSMutableArray alloc] init];
    
    for (NSString* sentence in arraySentence) {
        NSRange range = [sentence rangeOfString:@"NSString"];
        if (range.location != NSNotFound) {
            [arraySentenceWithString addObject:sentence];
        }
    }
    
    NSMutableArray* arraySentenceWordReverse = [[NSMutableArray alloc] init];
    
    for (NSString* sentence in arraySentenceWithString) {
        NSMutableString* newSentence = [[NSMutableString alloc] init];
        NSInteger length = [sentence length] - 1;
        NSRange searchRange = NSMakeRange(0, [sentence length]);
        while (YES) {
            NSRange range = [sentence rangeOfString:@" " options:NSBackwardsSearch range:searchRange];
            if (range.location != NSNotFound) {
                searchRange.length = range.location;
                NSRange word = NSMakeRange(range.location + 1, length - range.location - 1);
                [newSentence appendFormat:@"%@ ",[sentence substringWithRange:word]];
                length = searchRange.length;
            } else {
                NSRange word = NSMakeRange(0, length);
                [newSentence appendString:[sentence substringWithRange:word]];
                break;
            }
        };
        [arraySentenceWordReverse addObject:newSentence];
    }
    
    for (NSInteger i = 0; i < arraySentenceWithString.count; i++) {
        NSLog(@"%@", [arraySentenceWithString objectAtIndex:i]);
        NSLog(@"%@", [arraySentenceWordReverse objectAtIndex:i]);
    }
    */
    
    
    /*
    for (NSString* sentence in arraySentenceWordReverse) {
        NSLog(@"%@", sentence);
    }
    */
    
    NSMutableString* newString = [[NSMutableString alloc] init];
    [newString appendString:mainString];
    
    NSRange firstSymbolRange = NSMakeRange(0, 1);
    NSString* firstSymbol = [newString substringWithRange:firstSymbolRange];
    [newString replaceCharactersInRange:firstSymbolRange withString:[firstSymbol lowercaseString]];
    NSRange lastSymbolRange = NSMakeRange([newString length] - 2, 1);
    NSString* lastSymbol = [newString substringWithRange:lastSymbolRange];
    [newString replaceCharactersInRange:lastSymbolRange withString:[lastSymbol uppercaseString]];
    
    [self replaceSymbolsAroundSubstring:newString subString:@". "];
    [self replaceSymbolsAroundSubstring:newString subString:@", "];
    [self replaceSymbolsAroundSubstring:newString subString:@" "];
    [self replaceSymbolsAroundSubstring:newString subString:@") "];
    [self replaceSymbolsAroundSubstring:newString subString:@" ("];
...
- (void) replaceSymbolsAroundSubstring:(NSMutableString*)string subString:(NSString*)subString {
    NSRange searchRange = NSMakeRange(0, [string length]);
    while (YES) {
        NSRange range = [string rangeOfString:subString options:0 range:searchRange];
        if (range.location != NSNotFound) {
            NSRange lastSymbolRange = NSMakeRange(range.location - 1, 1);
            NSRange firstSymbolRange = NSMakeRange(range.location + range.length, 1);
            NSString* firstSymbol = [string substringWithRange:firstSymbolRange];
            NSString* lastSymbol = [string substringWithRange:lastSymbolRange];
            [string replaceCharactersInRange:lastSymbolRange withString:[lastSymbol uppercaseString]];
            [string replaceCharactersInRange:firstSymbolRange withString:[firstSymbol lowercaseString]];
